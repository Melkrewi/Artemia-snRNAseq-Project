...
set.seed(12345)
seurat_obj <- readRDS('../ATAC_integrated.rds')
...
seurat_obj$germline_pseudotime <- ifelse(seurat_obj$seurat_clusters %in% c("1", "3","6"), seurat_obj$pseudotime, NA)
...
p2 + p1
dev.off()
export_df <- seurat_obj@meta.data %>%
  rownames_to_column("barcodes") %>%
  select(barcodes, seurat_clusters,germline_pseudotime,nCount_RNA,nCount_peaks)
write.csv(export_df, "pseudotime_and_others.txt")
